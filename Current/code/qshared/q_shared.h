...
#define	Q2E_VERSION		"Q2E 1.0 Alpha"
...
#ifndef M_PI2
#define M_PI2					6.28318530717958647692f		// Matches value in GCC v2 math.h
#endif
...
#define VectorClamp(a,b,c)		((a)[0]=((a)[0]<(b)[0])?(b)[0]:((a)[0]>(c)[0])?(c)[0]:(a)[0],((a)[1]=((a)[1]<(b)[1])?(b)[1]:((a)[1]>(c)[1])?(c)[1]:(a)[1],((a)[2]=((a)[2]<(b)[2])?(b)[2]:((a)[2]>(c)[2])?(c)[2]:(a)[2])
...
#define VectorMin(a,b,o)		_VectorMin(a, b, o)
#define VectorMax(a,b,o)		_VectorMax(a, b, o)
#define VectorClamp(a,b,c)		_VectorClamp(a, b, c)
...
#define VectorLerp(a,b,f,o)		_VectorLerp(a,b,f,o)
...
int			SphereOnPlaneSide (const vec3_t center, float radius, const struct cplane_s *plane);
int			PointOnPlaneSide (const vec3_t point, float epsilon, const struct cplane_s *plane);
...
// Removes Quake color sequences from string
char		*Q_CleanStr (char *string);

// String compare for use in qsort calls
...
typedef struct cvar_s {
	...
	char			*resetValue;	// cvar_restart and reset will set this value
	char			*latchedValue;	// For CVAR_LATCH variables
	int				flags;			// CVAR_? flags
	char			*description;	// Variable description
	qboolean		modified;		// Set each time the variable is changed
...
#define	ANGLE2SHORT(x)	((int)((x)*65536/360.0) & 65535)
