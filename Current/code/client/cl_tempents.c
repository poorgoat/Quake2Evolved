...
void CL_ParseTempEntity (void){
	...
	case TE_GUNSHOT:		// Bullet hitting wall
		...
		CL_ProjectDecal(pos, dir, 3, cl.media.bulletMarkMaterial);
	...
	case TE_BLUEHYPERBLASTER:
		...
		CL_ProjectDecal(pos, dir, 3, cl.media.energyMarkMaterial);
	...
	case TE_RAILTRAIL:		// Railgun effect
		...
		CL_ProjectDecal(pos2, dir, 3, cl.media.energyMarkMaterial);
	...
	case TE_GRENADE_EXPLOSION_WATER:
		...
		if (teType != TE_GRENADE_EXPLOSION_WATER){
			CL_Explosion(pos, dir, 40, rand() % 360, 350, 1.0, 0.5, 0.5, cl.media.grenadeExplosionMaterial);
			CL_ExplosionParticles(pos);
			CL_ProjectDecal(pos, dir, 40, cl.media.burnMarkMaterial);
			S_StartSound(pos, 0, 0, cl.media.sfxGrenadeExplosion, 1, ATTN_NORM, 0);
		}
		else {
			CL_Explosion(pos, dir, 40, rand() % 360, 350, 1.0, 0.5, 0.5, cl.media.grenadeExplosionWaterMaterial);
	...
	case TE_ROCKET_EXPLOSION_WATER:
		...
		if (teType != TE_ROCKET_EXPLOSION_WATER){
			...
			CL_Explosion(pos, dir, 40, rand() % 360, 350, 1.0, 0.5, 0.5, cl.media.rocketExplosionWaterMaterial);
			CL_ExplosionWaterSplash(pos);
			CL_BubbleParticles(pos, 384, 30);
			CL_ProjectDecal(pos, dir, 40, cl.media.burnMarkMaterial);
			S_StartSound(pos, 0, 0, cl.media.sfxWaterExplosion, 1, ATTN_NORM, 0);
	...
	case TE_PLASMA_EXPLOSION:
		...
		if (teType != TE_EXPLOSION1_BIG){
			...
			CL_ExplosionParticles(pos);
			CL_ProjectDecal(pos, dir, 60, cl.media.burnMarkMaterial);
			S_StartSound(pos, 0, 0, cl.media.sfxRocketExplosion, 1, ATTN_NORM, 0);
		}
	...
	case TE_BFG_BIGEXPLOSION:
		...
		if (teType != TE_BFG_BIGEXPLOSION){
			...
			CL_Explosion(pos, vec3_origin, 40, 0, 350, 0.0, 1.0, 0.0, cl.media.bfgExplosionMaterial);
			CL_ProjectDecal(pos, dir, 40, cl.media.burnMarkMaterial);
		}
		else {
			// HACK!!!
			...
			CL_Explosion(pos, vec3_origin, 60, 0, 500, 0.0, 1.0, 0.0, cl.media.bfgExplosionMaterial);
	...
	case TE_NUKEBLAST:
		...
		for (i = 0; i < 5; i++){
			...
			CL_Explosion(pos2, dir, 90, rand() % 360, 0, 0, 0, 0, cl.media.rocketExplosionMaterial);
		...
		CL_ProjectDecal(pos, dir, 240, cl.media.burnMarkMaterial);
...
static void CL_AddBeams (void){
	...
	for (i = 0, b = cl_beams; i < MAX_BEAMS; i++, b++){
		...
		if (b->type != BEAM_HEAT){
			...
			if (b->entity == cl.clientNum && !cl_thirdPerson->integerValue){
			...
			if (b->entity == cl.clientNum && !cl_thirdPerson->integerValue){
				...
				VectorScale(cl.renderView.viewAxis[0], len, vec);
		...
		if (b->type != BEAM_PARASITE){
			...
			case BEAM_LIGHTNING:
				ent.beamWidth = 10;
				ent.beamLength = 30;
				ent.customMaterial = cl.media.lightningBeamMaterial;
				break;
			case BEAM_HEAT:
				ent.beamWidth = 2;
				ent.beamLength = 30;
				ent.customMaterial = cl.media.heatBeamMaterial;
				break;
			}
