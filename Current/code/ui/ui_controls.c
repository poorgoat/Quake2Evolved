...
static void UI_Controls_GetConfig (void){
	...
	uiControls.mouseSpeed.curValue = Cvar_GetInteger("sensitivity");
...
static void UI_Controls_SetConfig (void){
	...
	for (binding = uiControlsBindings; binding->id != -1; binding++){
		if (binding->bind1 != -1){
			CL_SetKeyBinding(binding->bind1, binding->command);
			
			if (binding->bind2 != -1)
				CL_SetKeyBinding(binding->bind2, binding->command);
		}
	}
	...
	if ((int)uiControls.invertMouse.curValue)
		Cvar_SetFloat("m_pitch", -Q_fabs(Cvar_GetFloat("m_pitch")));
	else
		Cvar_SetFloat("m_pitch", Q_fabs(Cvar_GetFloat("m_pitch")));
	...
	if (item->type == QMTYPE_ACTION){
		if (!uiControls.waitingForKey){
			...
			case K_KP_DEL:
				...
				if (binding->bind1 != -1){
					CL_SetKeyBinding(binding->bind1, "");
					binding->bind1 = -1;
				}
				if (binding->bind2 != -1){
					CL_SetKeyBinding(binding->bind2, "");
					binding->bind2 = -1;
				}
...
static void UI_Controls_Ownerdraw (void *self){
	...
	if (item->type == QMTYPE_ACTION){
		...
		if (binding->bind1 == -1)
			...
			Q_strncpyz(name, CL_KeyNumToString(binding->bind1), sizeof(name));
			Q_strupr(name);

			if (binding->bind2 != -1){
				Q_strncpyz(name2, CL_KeyNumToString(binding->bind2), sizeof(name2));
				Q_strupr(name2);
