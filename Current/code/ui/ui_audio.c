...
static void UI_Audio_GetDeviceList (void){
	...
	if (!uiStatic.alConfig.deviceList || !uiStatic.alConfig.deviceName){
		uiAudio.audioDeviceList[0] = "Generic Hardware";
		uiAudio.audioDeviceList[1] = "Generic Software";
...
static void UI_Audio_GetConfig (void){
	...
	if (!Cvar_GetInteger("cd_noCD"))
		uiAudio.cdMusic.curValue = 1;

	uiAudio.cdVolume.curValue = Cvar_GetFloat("cd_volume");

	uiAudio.masterVolume.curValue = Cvar_GetFloat("s_masterVolume");

	uiAudio.sfxVolume.curValue = Cvar_GetFloat("s_sfxVolume");

	uiAudio.musicVolume.curValue = Cvar_GetFloat("s_musicVolume");

	uiAudio.rolloffFactor.curValue = Cvar_GetFloat("s_rolloffFactor");

	uiAudio.dopplerFactor.curValue = Cvar_GetFloat("s_dopplerFactor");

	if (Cvar_GetInteger("s_eax"))
...
static void UI_Audio_SetConfig (void){
	...
	Cvar_SetFloat("cd_volume", uiAudio.cdVolume.curValue);

	Cvar_SetString("s_deviceName", uiAudio.audioDeviceList[(int)uiAudio.audioDevice.curValue]);

	Cvar_SetFloat("s_masterVolume", uiAudio.masterVolume.curValue);

	Cvar_SetFloat("s_sfxVolume", uiAudio.sfxVolume.curValue);

	Cvar_SetFloat("s_musicVolume", uiAudio.musicVolume.curValue);

	Cvar_SetFloat("s_rolloffFactor", uiAudio.rolloffFactor.curValue);

	Cvar_SetFloat("s_dopplerFactor", uiAudio.dopplerFactor.curValue);

	Cvar_SetInteger("s_eax", (int)uiAudio.eax.curValue);

	// Restart sound subsystem
	Cbuf_ExecuteText(EXEC_NOW, "s_restart\n");
}
