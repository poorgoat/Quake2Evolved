...
static void UI_AddressBook_SaveServers (void){
	...
	for (i = 0; i < 16; i++){
		...
		Cvar_SetString(name, uiAddressBook.servers[i].buffer);
	}
}
...
static void UI_AddressBook_Init (void){
	...
	for (i = 0, y = 128; i < 16; i++, y += 32){
		...
		Q_strncpyz(uiAddressBook.servers[i].buffer, Cvar_GetString(va("server%i", i+1)), sizeof(uiAddressBook.servers[i].buffer));
	}
...
static void UI_DMOptions_Callback (void *self, int event){
	...
	int					flags = Cvar_GetInteger("dmflags");
	int					bit = 0;
...
static void UI_StartServer_GetMapList (void){
...
#ifdef SECURE
	f = fopen_s(f, va("%s/%s/maps.lst", Cvar_GetString("fs_homePath"), Cvar_GetString("fs_game")), "rb");
#else
	f = fopen(va("%s/%s/maps.lst", Cvar_GetString("fs_homePath"), Cvar_GetString("fs_game")), "rb");
#endif
	if (!f){
		script = PS_LoadScriptFile("maps.lst");
		if (!script)
		...
		script = PS_LoadScriptMemory("maps.lst", buffer, size);
		if (!script){
			Z_Free(buffer);
			Com_Error(ERR_FATAL, "Couldn't find maps.lst");
		}
	...
	while (1){
		...
		Q_strncpyz(uiStartServer.maps[uiStartServer.numMaps], token.string, sizeof(uiStartServer.maps[uiStartServer.numMaps]));

		if (!PS_ReadToken(script, 0, &token))
			continue;	// Invalid

		Q_strncpyz(uiStartServer.names[uiStartServer.numMaps], token.string, sizeof(uiStartServer.names[uiStartServer.numMaps]));
...
static void UI_StartServer_Start (void){
	...
	Cvar_SetString("sv_hostName", uiStartServer.hostName.buffer);
...
static void UI_StartServer_Init (void){
	...
	Q_snprintfz(uiStartServer.timeLimit.buffer, sizeof(uiStartServer.timeLimit.buffer), "%i", Cvar_GetInteger("timelimit"));
	...
	Q_snprintfz(uiStartServer.fragLimit.buffer, sizeof(uiStartServer.fragLimit.buffer), "%i", Cvar_GetInteger("fraglimit"));
	...
	if (Cvar_GetInteger("maxclients") <= 1)
		Q_snprintfz(uiStartServer.maxClients.buffer, sizeof(uiStartServer.maxClients.buffer), "8");
	else
		Q_snprintfz(uiStartServer.maxClients.buffer, sizeof(uiStartServer.maxClients.buffer), "%i", Cvar_GetInteger("maxclients"));
	...
	Q_strncpyz(uiStartServer.hostName.buffer, Cvar_GetString("sv_hostName"), sizeof(uiStartServer.hostName.buffer));
