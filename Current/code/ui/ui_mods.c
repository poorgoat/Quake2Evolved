...
typedef struct {
	char					modsDir[MAX_MODS][MAX_OSPATH];
...
static void UI_Mods_GetModList (void){
	...
	if (!Q_stricmp(Cvar_GetString("fs_game"), uiMods.modsDir[0]))
...
static void UI_Mods_Callback (void *self, int event){
	...
	if (event == QM_CHANGED){
		// See if the load button should be grayed
		if (!Q_stricmp(Cvar_GetString("fs_game"), uiMods.modsDir[uiMods.modList.curItem]))
			uiMods.load.generic.flags |= QMF_GRAYED;
		else
			uiMods.load.generic.flags &= ~QMF_GRAYED;
	...
	case ID_LOAD:
		...
		FS_Restart(uiMods.modsDir[uiMods.modList.curItem]);
