...
typedef struct {
	char	mission[MAX_OSPATH];
	char	name[80];
	char	objectives[1024];
	char	levelShots[MAX_LEVELSHOTS][MAX_OSPATH];
	int		numLevelShots;
} missionInfo_t;
...
static void UI_SinglePlayer_GetMissionList (void){
	...
	Q_snprintfz(name, sizeof(name), "scripts/ui/mission_%s.txt", Cvar_GetString("fs_game"));
	...
	while (1){
		...
		Q_strncpyz(uiSinglePlayer.missions[uiSinglePlayer.numMissions].mission, token.string, sizeof(uiSinglePlayer.missions[uiSinglePlayer.numMissions].mission));
		...
		while (1){
			...
			if (!Q_stricmp(token.string, "name")){
				...
				Q_strncpyz(uiSinglePlayer.missions[uiSinglePlayer.numMissions].name, token.string, sizeof(uiSinglePlayer.missions[uiSinglePlayer.numMissions].name));
			}
			else if (!Q_stricmp(token.string, "objective")){
				PS_ReadToken(script, PSF_ALLOW_NEWLINES | PSF_ALLOW_STRINGCONCAT | PSF_ALLOW_ESCAPECHARS, &token);
				Q_strncpyz(uiSinglePlayer.missions[uiSinglePlayer.numMissions].objectives, token.string, sizeof(uiSinglePlayer.missions[uiSinglePlayer.numMissions].objectives));
			}
			else if (!Q_stricmp(token.string, "levelshots")){
				char	*levelshot;
				...
				for (i = 0; i < MAX_LEVELSHOTS; i++){
					...
					Q_strncpyz(levelshot, token.string, MAX_OSPATH);
...
static void UI_SinglePlayer_StartGame (void){
	...
	char	*game = Cvar_GetString("fs_game");
	char	text[256];
