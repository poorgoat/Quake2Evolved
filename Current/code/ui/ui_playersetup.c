...
typedef struct {
	char					playerModels[MAX_PLAYERMODELS][MAX_OSPATH];
	int						numPlayerModels;
	char					playerSkins[MAX_PLAYERSKINS][MAX_OSPATH];
	int						numPlayerSkins;
	char					currentModel[MAX_OSPATH];
	char					currentSkin[MAX_OSPATH];
...
static void UI_PlayerSetup_FindModels (void){
	...
	char	list[MAX_PLAYERMODELS][MAX_OSPATH];
	int		i, count = 0;
	...
	for (i = 0; i < numFiles; i++){
		...
		Q_strncpyz(list[count], fileList[i], sizeof(list[count]));
		count++;
	}
...
static void UI_PlayerSetup_FindSkins (const char *model){
	...
	char	list[MAX_PLAYERSKINS][MAX_OSPATH];
	int		i, count = 0;

	uiPlayerSetup.numPlayerSkins = 0;

	// Get file list
	fileList = FS_ListFiles(va("players/%s", model), NULL, true, &numFiles);

	for (i = 0; i < numFiles; i++){
		if (!strrchr(fileList[i], '.'))
			continue;

		if (strstr(fileList[i], "_i."))
			continue;

		if (!strstr(fileList[i], ".pcx") && !strstr(fileList[i], ".tga"))
			continue;
...
static void UI_PlayerSetup_GetConfig (void){
	...
	Q_strncpyz(uiPlayerSetup.name.buffer, Cvar_GetString("name"), sizeof(uiPlayerSetup.name.buffer));

	// Get user set skin
	Q_strncpyz(model, Cvar_GetString("skin"), sizeof(model));
	ch = strchr(model, '/');
	if (!ch)
		ch = strchr(model, '\\');
	if (ch){
...
static void UI_PlayerSetup_SetConfig (void){
	...
	Cvar_SetString("name", uiPlayerSetup.name.buffer);

	Cvar_SetString("skin", va("%s/%s", uiPlayerSetup.currentModel, uiPlayerSetup.currentSkin));
}
...
static void UI_PlayerSetup_Ownerdraw (void *self){
	...
	if (item->id == ID_VIEW){
		...
		ent.customMaterial = R_RegisterMaterial(path, true);
		...
		ent.materialParms[MATERIALPARM_TIME_OFFSET] = 0.0;
		ent.materialParms[MATERIALPARM_DIVERSITY] = 0.0;
		ent.materialParms[MATERIALPARM_GENERAL] = 0.0;
		...
		// Add a light
		VectorSet(light.origin, 0, -50, 0);
		VectorClear(light.center);
		AxisClear(light.axis);
		VectorSet(light.radius, 200, 200, 200);
		light.style = 0;
		light.detailLevel = 10;
		light.parallel = false;
		light.noShadows = false;
		light.customMaterial = NULL;
		light.materialParms[MATERIALPARM_RED] = 1.0;
		light.materialParms[MATERIALPARM_GREEN] = 1.0;
		light.materialParms[MATERIALPARM_BLUE] = 1.0;
		...
		memset(&renderView, 0, sizeof(renderView_t));
		...
		AxisCopy(axisDefault, renderView.viewAxis);
