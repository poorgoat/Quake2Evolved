...
static void SV_New_f (void){
	...
	if (sv.state == SS_DEMO){
		...
		FS_OpenFile(name, &sv.demoFile, FS_READ);
		if (!sv.demoFile)
	...
	MSG_WriteString(&sv_client->netChan.message, Cvar_GetString("fs_game"));
	...
	if (sv.state == SS_GAME){
		...
		sv_client->edict = ent;
		memset(&sv_client->lastCmd, 0, sizeof(usercmd_t));
...
static void SV_Download_f (void){
	...
	if (strstr(name, "..") || !strstr(name, "/") || name[0] == '.' || name[0] == '/' || !sv_allowDownload->integerValue){
		MSG_WriteByte(&sv_client->netChan.message, SVC_DOWNLOAD);
	...
	sv_client->downloadSize = FS_OpenFile(name, &sv_client->downloadFile, FS_READ);
...
static void SV_ClientThink (client_t *cl, usercmd_t *cmd){
	...
	if (cl->commandMsec < 0 && sv_enforceTime->integerValue){
